...
export function goalTool(name) {
  ...
  if (report) return { action: report, contract: GOAL_NETWORK_REPORT_CONTRACT, parse: parseGoalNetworkReportInput, valid: validGoalNetworkReportResult,
    write: goalNetworkReportIsWrite(report), operationTool: 'aidesk_goal_network_report_operation' };
...
function validReportCore(value, input) {
  return validGoalNetworkReportResult('report', { contract: GOAL_NETWORK_REPORT_CONTRACT, status: 'recorded', receipt: value }, input)
    && sameGoalValue(value.request, input);
}
...
export function completeGoalOperation(store, row, receipt, now) {
  ...
  if (tool.contract === GOAL_NETWORK_REPORT_CONTRACT) {
    ...
    needGoal(validReportCore(receipt, row.entry.input), 'RECEIPT_INVALID');
  } else if (tool.contract === GOAL_NETWORK_SCOPE_CONTRACT) {
